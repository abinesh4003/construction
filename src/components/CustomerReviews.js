      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-bold text-amber-400 font-playfair mb-4 md:mb-6">
            <span className="font-inter text-white">Our Happy </span> Customers
          </h2>
          <p className="text-base md:text-lg text-gray-200 max-w-3xl mx-auto px-4 inter">
        ...
        <div className="relative">
          ...
          <button
            onClick={handlePrev}
            className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 p-2 rounded-full shadow-md transition-all z-10"
          >
            <ChevronLeft className="h-5 w-5 md:h-6 md:w-6 text-amber-400" />
          </button>
          <button
            onClick={handleNext}
            className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 p-2 rounded-full shadow-md transition-all z-10"
          >
          ...
          <AnimatePresence custom={direction} mode="popLayout">
            ...
            >
              {visibleReviews.map((review) => (
                <motion.div
                  key={review.id}
                  className="bg-white/10 backdrop-blur-xl p-6 md:p-8 border border-gray-200 rounded-3xl flex flex-col h-full shadow-2xl hover:scale-105 transition-transform"
                ...
                >
                  ...
                  <div className="flex items-center mb-4 md:mb-6">
                    <div className="flex mr-3 md:mr-4">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`h-4 w-4 md:h-5 md:w-5 ${i < review.rating ? "text-amber-400 fill-amber-400" : "text-gray-600"}`}
                        />
                      ))}
                    </div>
                    <Quote className="h-6 w-6 md:h-8 md:w-8 text-amber-400 opacity-20" />
                  </div>

                  {/* Content */}
                  <div className="flex-grow mb-4 md:mb-6">
                    <p className="text-gray-200 italic text-sm md:text-base inter relative pl-4 md:pl-6 before:absolute before:left-0 before:top-0 before:h-full before:w-px before:bg-amber-400">
                      &quot;{review.content}&quot;
                    </p>
                  </div>
                  ...
                  <div className="flex items-center mt-auto">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="montserrat font-semibold text-white text-base md:text-lg truncate">
                        {review.name}
                      </h4>
                      <p className="inter text-gray-300 text-xs md:text-sm truncate">
                        {review.role}
                      </p>
                    </div>
                  </div>
          ...
          <div className="flex justify-center mt-8 md:mt-12 gap-2">
            {reviews.map((_, index) => (
              <button
                ...
                className={`h-2 w-2 md:h-3 md:w-3 rounded-full transition-all ${currentIndex === index ? "bg-amber-400 md:w-6 w-4" : "bg-gray-500"}`}
